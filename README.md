...
Сохранение выполняется при нажатии `Result`, а также при паузе/закрытии приложения.  
История ограничена (`MaxHistoryEntries = 1000`) для контроля памяти и размера сохранений.  
Запись в файл атомарна (временный файл + rename). При проблемах чтения используется резервная копия (`.bak`).
